# Chrome Web Store listing — Quarto
...
Quarto brings order to a noisy tab strip without sending anything about your
...
- Smart group suggestions. Domain bucketing plus a token-similarity pass
  ...
  your workflow.
- Add-to-existing-group hints. Open a new tab and Quarto scores it against
...
- No content scripts on your pages. Quarto only reads the tab metadata Chrome
...
- sidePanel: Render the primary Quarto surface in Chrome's side panel.
...
- host permissions (<all_urls>): Required so tab and group reading works on any
  site the user has open. Quarto reads only Chrome-exposed metadata; it does
  not inject scripts into or read the content of web pages.

## Single purpose (required field)
Quarto organizes a user's open tabs: grouping related tabs, attaching notes
